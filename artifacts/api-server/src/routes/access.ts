...
router.get("/access-requests/me", requireAuth, async (req, res) => {
  try {
    ...
    req.log.error({ err }, "Failed to fetch access status");
    res.status(500).json({ error: dbErrorMessage(err) });
  }
});

// ---- Admin ----
router.get("/admin/access-requests", requireAuth, requireSuperAdmin, async (req, res) => {
  try {
    const rows = await db.select().from(accessRequestsTable).orderBy(accessRequestsTable.createdAt);
    res.json(rows);
  } catch (err) {
    req.log.error({ err }, "Failed to fetch access requests");
    res.status(500).json({ error: dbErrorMessage(err) });
  }
});
...
router.get("/admin/teams", requireAuth, requireSuperAdmin, async (req, res) => {
  try {
